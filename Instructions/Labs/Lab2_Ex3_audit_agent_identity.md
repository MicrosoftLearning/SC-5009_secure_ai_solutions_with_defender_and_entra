---
lab:
  title: Exercise - Verify and audit AI workload access
  module: Secure access for AI workloads
  description: Verify API permissions, audit role assignments, and rotate application credentials.
  duration: 15 minutes
  level: 200
  islab: true
  primarytopics:
...
1. Review the application's API permissions and consent status
1. Review the enterprise application properties
1. Review Azure activity logs for role assignments
1. Rotate the application client secret
...
1. On the **Permissions** page, review the permissions listed. You should see **User.Read.All** under **Microsoft Graph** with a consent status of **Not granted**.
    ...
    > **Note**: The "Not granted" status means this permission is configured but not yet active. In production, a Global Administrator grants admin consent before the permission can be used. This consent workflow is a security checkpoint that prevents applications from accessing data without approval.
...
You've successfully verified the application's API permission configuration and consent status.

## Task 2 – Review the enterprise application properties

The enterprise application properties control how the service principal behaves in your tenant. In this task, you'll review key settings that affect sign-in behavior and access control.
...
1. On the **Properties** page, review the following settings:
   ...
   - **Assignment required**: No
    ...
    > **Note**: When **Assignment required** is set to **No**, any user or service in the tenant can obtain tokens for this application. In production, setting this to **Yes** restricts token issuance to only explicitly assigned users or groups. This helps limit which identities can use the application.

1. Consider whether this configuration allows unrestricted token issuance, and when you would change **Assignment required** to **Yes**.

You've successfully reviewed the enterprise application properties.

## Task 3 – Review Azure activity logs for role assignments

Azure activity logs capture management operations on Azure resources, including role assignments. In this task, you'll verify that the RBAC role assignments granted to the application's service principal were recorded.
...
1. In the **Add a client secret** flyout, enter:
   ...
   - **Description**: `Rotated application credential`
...
1. Verify that both secrets are now listed — the original and the newly created one.
    ...
    > **Note**: During production rotation, this overlap period is critical. The new credential must be distributed to all dependent services before the old one is removed. Deleting the old secret too early will break authentication.

1. On the original **AI workload authentication** secret, select the delete icon, then confirm the deletion.

1. Verify that only the **Rotated application credential** secret remains.

You've successfully rotated the client secret using the recommended workflow: create new, verify overlap, remove old.
